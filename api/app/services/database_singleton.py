...
class CosmosDBService:
    ...
    @classmethod
    def update_item(cls, item_id, updated_item, container_id):
        if container_id not in cls.containers:
            raise Exception(
                f"Container {container_id} does not exist or has not been initialized.")
        container = cls.containers[container_id]
        # Simply replace the item with the updated version
        updated_item = container.replace_item(item=item_id, body=updated_item)
        return updated_item
